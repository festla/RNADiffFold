...
class Experiment(DiffusionExperiment):
    ...
    def train_fn(self, epoch):
        ...
        pdb.set_trace()
        """
        for _, batch in enumerate(pbar):    # self.train_loader
            if batch is None:
                continue
            ...
            # print("DEBUG loss.requires_grad =", loss.requires_grad, "type =", type(loss))
            loss.backward()
    ...
    def val_fn(self, epoch):
        ...
        with torch.no_grad():
            ...
            val_iter = tqdm(self.val_loader, desc=f'Val epoch {epoch+1}', leave=True, dynamic_ncols=True, file=sys.stdout)
            for _, batch in enumerate(val_iter):
                ...
                pred_x0, _ = self.model.sample(batch_size, data_fcn_2, data_seq_raw, set_max_len, contact_masks, data_seq_encoding, show_progress=False)
